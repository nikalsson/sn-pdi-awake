...
if (!process.env.INSTANCENAME) {
  writeToLog("INSTANCE NOT KNOWN, EXITING!");
  process.exit();
}
...
async function getLog() {
  try {
    ...
    if (response.status == 401) {
      ...
      console.log(`getLog response.data: ${JSON.stringify(response.data)}`)
...
async function createLogEntry() {
  try {
    ...
    if (minutesSinceLastGetReq > 14) {
      ...
      const logContent = new Date().toISOString().replace(/[TZ]/g, " ") + " >> " + 'PING FROM RESTOperations.js';
